...
Pour généré la les plongements, j'ai utilisé les programmes proposés à l'adresse suivante : 
<https://github.com/stanfordnlp/GloVe> (recommandé par Achille Lacroix).
...
Pour chercher les potentiels biais de genre, j'ai écris un code pour afficher les dix voisins les plus proches du vecteur correspondant à `token - homme + femme`. (cf. fichier `code.py`)

## Cas interressants

```python
# status hiérarchiques
